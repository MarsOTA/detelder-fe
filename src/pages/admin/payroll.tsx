...
const payroll = () => {
  ...
  const caricaPayroll = async (filtri: FiltriRicerca) => {
    ...
    const resp = await fetch(`${ezystaffBEUrl}payroll?${queryParams.toString()}`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
        "Content-Type": "application/json",
        accept: "application/json",
  ...
  const calcolaMinutiDefinitivi = (oraInizio: string, oraFine: string, oraPausa: number = 0): number => {
    ...
    return Math.max(0, Math.round(fine - inizio - (Number(oraPausa) || 0) * 60));
  };

  const formatMinuti = (minutiTotali: number): string =>
    `${String(Math.floor(minutiTotali / 60)).padStart(2, "0")}:${String(minutiTotali % 60).padStart(2, "0")}`;

  const calcolaTotaleOre = (oraInizio: string, oraFine: string, oraPausa: number = 0): string =>
    formatMinuti(calcolaMinutiDefinitivi(oraInizio, oraFine, oraPausa));
  ...
  const setOraInizioTurno = (idTurno: number, ora: string) =>
    setTurni((prev) => prev.map((t) => (t.idTurno === idTurno ? { ...t, oraInizioDefinitivo: ora } : t)));

  const setOraFineTurno = (idTurno: number, ora: string) =>
    setTurni((prev) => prev.map((t) => (t.idTurno === idTurno ? { ...t, oraFineDefinitivo: ora } : t)));
    ...
    <section className="m-6" style={{ fontFamily: "'Mulish', sans-serif" }}>
      ...
      <div className="flex items-center bg-transparent mb-1">
        <div className="flex items-center bg-transparent p-4 mb-1">
          ...
          <Button className="bg-[#5e8a7a] hover:bg-[#5e8a7a] hover:text-white cursor-pointer rounded-r-full rounded-l-none -ml-px" onClick={() => filtriRicerca && caricaPayroll(filtriRicerca)}>Filtra</Button>
        </div>

        <div className="flex gap-3">
          <Button onClick={() => handlePeriodoClick("giorno")} className="rounded-[18px] border border-[#007a55] bg-[#f3fffa] text-[#007a55] text-[16px] font-bold cursor-pointer hover:bg-[#e6fff5] hover:text-[#007a55]">Ieri</Button>
          <Button onClick={() => handlePeriodoClick("settimana")} className="rounded-[18px] border border-[#007a55] bg-[#f3fffa] text-[#007a55] text-[16px] font-bold cursor-pointer hover:bg-[#e6fff5] hover:text-[#007a55]">Settimana</Button>
          <Button onClick={() => handlePeriodoClick("mese")} className="rounded-[18px] border border-[#007a55] bg-[#f3fffa] text-[#007a55] text-[16px] font-bold cursor-pointer hover:bg-[#e6fff5] hover:text-[#007a55]">Mese</Button>
          <div className="h-8 w-px bg-[#c4c7c5]" />
          <Button onClick={() => filtraPerStatoElaborazione("ELABORATO")} className="rounded-full border border-[#9df5c3] bg-[#9df5c3] text-[#002112] text-[13px] font-semibold cursor-pointer hover:bg-[#9df5c3] hover:text-[#002112]">Elaborati</Button>
          <Button onClick={() => filtraPerStatoElaborazione("DA_ELABORARE")} className="rounded-full border border-[#fed7aa] bg-[#ffedd5] text-[#c2410c] text-[13px] font-semibold cursor-pointer hover:bg-[#ffedd5] hover:text-[#c2410c]">Da Elaborare</Button>
          <Button onClick={() => filtraPerStatoElaborazione("MODIFICATO")} className="rounded-full border border-[#c3cfeb] bg-[#d5e2ff] text-[#4c608b] text-[13px] font-semibold cursor-pointer hover:bg-[#d5e2ff] hover:text-[#4c608b]">Modificato</Button>
                ...
                <React.Fragment key={index}>
                  ...
                  <TableRow className="h-full">
                    ...
                    <TableCell>
                      ...
                      {turno.statoPayroll === "MODIFICATO" && <Button disabled className="w-full cursor-not-allowed rounded-[5px] bg-[#d5e2ff] text-[11px] font-bold text-[#4c608b] hover:bg-[#d5e2ff] hover:text-[#4c608b]">Modificato</Button>}
                    </TableCell>
                  </TableRow>
                </React.Fragment>
              );
